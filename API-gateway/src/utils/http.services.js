...
module.exports = {
    ...
    getUserProducts: async function ({ userId, productCategory }) {
        const productsResponse = await productServerURL.get(`/products/${userId}`);
        return productsResponse.data;  
    },
    getSimilarProducts: async function ({
        userId, 
        userEmail, 
        productCategory
    }) {
        const getSimilarProducts = await productServerURL.get(`/similar-products/${userId}/${userEmail}/${productCategory}`);
        return getSimilarProducts.data;
    },
